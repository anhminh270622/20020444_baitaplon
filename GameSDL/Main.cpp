...
bool InitData()
{
	...
	g_window = SDL_CreateWindow("PHAM ANH MINH : 20020444  ",
	...
	else
	{
		...
		font_time = TTF_OpenFont("font//dlxfont_.ttf", 16);
		if (font_time == NULL)
		{
			success = false;
		}
	...
	g_sound_bullet[0] = Mix_LoadWAV("Gun+Empty.wav");

	g_sound_exp[0] = Mix_LoadWAV("Explosion+5.wav");
	if (g_sound_exp[0] == NULL || g_sound_bullet[0] == NULL ) {
		return false;
	}
...
int main(int argc, char* argv[])
{
	...
	while (!is_quit)
	{
		...
		ColorData color_data2(255, 255, 255);// viền thanh hiển thị
		...
		if (money_count == 30 && mark_value == 40) {
			TextObject win;
			win.SetColor(TextObject::RED_TEXT);
			std::string win_text("YOU WIN");
			win.SetText(win_text);
			win.LoadFromRenderText(font_time, g_screen);
			win.RenderText(g_screen, SCREEN_WIDTH * 0.5 - 50, 300);
		}
		...
		if (real_imp_time < time_one_frame)
		{
			...
			if (delay_time >= 0)
			{
				SDL_Delay(delay_time); //Nếu delay càng lớn thì chuyển động càng chậm
			}
		}
	}
